...
function Invoke-AuthRequest {
    ...
    $response = Invoke-RestMethod -Uri "http://localhost:4000/api/auth/$Endpoint" `
        -Method Post `
        -Body $json `
...
# Очищаем базу данных и создаем тестовые данные
Write-Host "Пересоздаем тестовые данные..."
